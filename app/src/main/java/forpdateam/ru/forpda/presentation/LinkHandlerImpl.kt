...
class LinkHandlerImpl @Inject constructor(
    ...
    private fun handleLink(link: Link, args: Map<String, String?>): Boolean {
        val screen = when (link) {
            is Link.Board.Announce -> Screen.Announce(link = link)
            is Link.Board.Auth.LoginForm -> Screen.Auth()
            is Link.Board.Favorite -> Screen.Favorites(link = link)
            is Link.Board.Forum -> Screen.Topics(link = link)
            is Link.Board.Mentions -> Screen.Mentions(link = link)
            is Link.Board.Profile -> Screen.Profile(link = link)
            is Link.Board.Qms.BlackList -> Screen.QmsBlackList()
            is Link.Board.Qms.Chat -> Screen.QmsChat.Created(link)
            is Link.Board.Qms.Contacts -> Screen.QmsContacts()
            is Link.Board.Qms.CreateThread -> Screen.QmsChat.Create(link = link)
            is Link.Board.Qms.Threads -> Screen.QmsThemes(link = link)
            is Link.Board.Reputation.History -> Screen.Reputation(link = link)
            is Link.Board.Reputation.Rating -> null // not supported
            is Link.Board.Root -> Screen.Forum()
            ...
            is Link.DevDb.Devices -> Screen.DevDbDevices(link = link)
            is Link.DevDb.Brands -> Screen.DevDbBrands(link = link)
            is Link.DevDb.Categories -> Screen.DevDbBrands(link = null)
            is Link.DevDb.Device -> Screen.DevDbDevice(link = link)
            is Link.DevDb.Search -> Screen.DevDbSearch(link = link)
            is Link.Site.Details -> Screen.ArticleDetail.FromLink(link = link)
            is Link.Site.Page -> Screen.ArticleList(link = link)
