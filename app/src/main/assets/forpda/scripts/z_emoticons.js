(function (q, r, s) {
    (function (n, p, k) {
        var h = {
            ...
            l, d = {
                basepath: "http://s.4pda.to/img/emot/",
                test_regexp: 0,
                buildRegexp: function (c, a) {
                    l = {};
                    a = [];
                    for (c in h) {
                        c += "";
                        var b = d.htmlspecialchars(c);
                        l[b] = c;
                        ":" != b.substr(0, 1) || ":" != b.substr(-1) ? a.push("(?:^| |\\n|\\r)" + b.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&") + "(?: |\\n|\\r|$)") : a.push(b.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&"))
                    }
                    a.sort(function (b, a) {
                        return (b + "").length > (a + "").length ? -1 : 1
                    });
                    d.test_regexp = RegExp(a.join("|"), "g")
                },
                ...
                parse: function (c, a) {
                    if (!a && c) a = c;
                    else if (a && c) d.basepath = c;
                    else return;
                    var b = a.firstChild,
                        e = k,
                        f = d.test_regexp,
                        g, m;
                    for (f || d.buildRegexp(); b;) {
                        if (3 !== b.nodeType) - 1 == (" " + b.className + " ").replace(/[\n\t]/g, " ").toLowerCase().indexOf(" noemoticons ") ? d.parse(b) : d.removeClass(b, "noemoticons");
                        else if (g = d.htmlspecialchars(b.nodeValue)) g = g.replace(d.test_regexp,
                            function (b, a, c) {
                                c = b.replace(/^\s+|\s+$/g, "");
                                a = h[c];
                ...
                parseAll: function (c) {
                    var a = p.querySelectorAll(".emoticons"),
                        b = a.length,
                        e = 0,
                        f = d.test_regexp;
                    f || d.buildRegexp();
        ...
        n.jsEmoticons = d;
        "$4" in n && $4.addModule("jsEmoticons", function () {
            return d
        })
    })(q, r)
})(window, document);
